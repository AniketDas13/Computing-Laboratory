...
int main()
{
    ...
    Facility *facility = (Facility *)malloc(m * sizeof(Facility));

    for (int i = 0; i < n; i++)
    {
        facility[i].next_time = 0;
    }
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        insert(&h, patient[i].curr_time[0]);
    ...
    while (h.num_used > 0)
    {
        ...
        for (int i = 0; i < m; i++)
        {
            for (int j = 0; j < patient[i].services; j++)
            {
                ...
                if (patient[i].curr_time[j] == time)
                {
                    if (patient[i].curr_time[j] < facility[patient[i].facility[j]].next_time)
                    {
                        patient[i].waiting_time += facility[patient[i].facility[j]].next_time - patient[i].curr_time[j];
                        patient[i].curr_time[j] = facility[patient[i].facility[j]].next_time;
                        insert(&h, patient[i].curr_time[j]);
                        ...
                        printf("%02d%02d: Patient %d needs Facility %d for %d minutes, waits\n", hours, minutes, i, patient[i].facility[j], patient[i].duration[j]);
                        continue;
                    }

                    facility[patient[i].facility[j]].next_time = patient[i].curr_time[j] + patient[i].duration[j];
                    ...
                    printf("%02d%02d: Facility %d attends to Patient %d for %d minutes\n", hours, minutes, patient[i].facility[j], i, patient[i].duration[j]);

                    if (j < patient[i].services - 1)
                    {
                        patient[i].curr_time[j + 1] = patient[i].curr_time[j] + patient[i].duration[j];
                        insert(&h, patient[i].curr_time[j + 1]);
                    }
                }
            }
        }
