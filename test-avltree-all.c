...
int delete(TREE *tree, int parent, int *root, DATA data);
...
void free_up_node(TREE *tree, int node);
...
int search(TREE *tree, int node, DATA data)
{
    ...
    else if (data < tree->nodelist[node].data)
    {
        return search(tree, tree->nodelist[node].left, data); // Search in the left subtree
    }
    else
    {
        return search(tree, tree->nodelist[node].right, data); // Search in the right subtree
    }
}
...
void free_up_node(TREE *tree, int node)
{
    tree->nodelist[node].parent = tree->free_list;
    tree->free_list = node;
    tree->num_nodes--;
}

/* Delete a node from the AVL tree */
int delete(TREE *tree, int parent, int *root, int data)
{
    if (*root == -1)
    ...
    if (data < tree->nodelist[thisnode].data)
    {
        return delete (tree, thisnode, &tree->nodelist[thisnode].left, data);
    }
    else if (data > tree->nodelist[thisnode].data)
    {
        return delete (tree, thisnode, &tree->nodelist[thisnode].right, data);
    }
    else
    {
        ...
        else
        {
            ...
            int child = (tree->nodelist[thisnode].left != -1) ? tree->nodelist[thisnode].left : tree->nodelist[thisnode].right;
            *root = child;
            if (child != -1)
            {
                tree->nodelist[child].parent = parent;
            }
