...
// int compare_marks_float(const void *a, const void *b)
// {
//     const struct Student *pa = (const struct Student *)a;
//     const struct Student *pb = (const struct Student *)b;
...
int main()
{
    ...
    // Eg: int compare(const void *a, const void *b)
    //     { return (*(int *)a - *(int *)b); }
    ...
    // float arr[] = {12.0, 13.5, 10.2, 1.6, 2.4, 18.5};

    // qsort(arr, 6, sizeof(float), compare);
    ...
    // float key = 13.5;
    // float *res = bsearch(&key, arr, 6, sizeof(float), compare);
